...
The `v0.2.4..v0.2.5` spec diff is two files and 168 added lines.
`[os.net.listen.opts]` (`net_listen_with`), `[os.net.wait]` (`net_wait`),
`[os.proc.inherit]` (`os_spawn_with`, `net_adopt_listener`) and `[os.cpus]`
(`os_cpus`) are five builtin FUNCTIONS; `[os.proc]` is a section header
declaring none. No new type, no new keyword, no new diagnostic code, no new LSP
...
repository has carried since le01 (the six captured editor smokes, stuck at
pin `70bdd35`) comes down to one.
...
parsing every changed record and comparing field by field. So
`v0.2.3..v0.2.4` moved nothing about the server's wire behaviour, capability
answers included.

That is why it was not the end of the work. v0.2.4's one change
...
bound a byte, so the sweep had nothing to notice. This sprint vendors
...
bs`), the type as WRITTEN instead of inferred (a `List[byte]` parameter),
`[type.byte.cast]`'s widening from outside (`bs[5] as int` → `int`), and
hover on the builtin type NAME, which answers `null`. That last one is
recorded so a pin that documents builtins moves this record.
...
The MATRIX row was itself the bug that file exists to prevent. It
...
re-recorded, only re-CAPTURED by driving that editor again. At le06 all
six sat at `70bdd35` and `lspconf replay` skipped all six. Five were driven
here.

- nvim: `nvim --headless`, NVIM v0.12.5, 7/7 assertions passing
  while recording. The old capture's `initialize` answer had no
  `completionProvider`, so it was materially wrong about what the server
...
- helix: driven through a pty, helix 25.07.1, the version the
...
- vscode: 16/16, and the server half of that suite had never run
  ...
  - `extension.test.ts` compared the whole of `wolf --version` against
    ...
    lane had skipped on every pin since the second line was added, never
    on the stale binary the check exists to catch. Found because the
...
wolf-lsp#7 is CLOSED. Two captures that leaked a home directory and could
not be re-recorded were waived at le06, which recorded that nomad-1 could not
...
test's own retirement clause forced the cleanup.
...
2. facsimile sends exactly one `didChange` per session. This is not a driver
...
were all re-verified in the source instead of carried forward.
...
shape a human checking "did the release land?" misreads. Recorded in
...
The upstream pin advances to `75fd2d0`, the `v0.2.1` release tag, and is
the first pin this repo has ever placed *at* a tag instead of on an
...
Two things about that string were measured, and both
...
re-recording is a one-line diff, with only the header's date and
`wolf_pin` moving, so the server's wire behaviour is byte-identical across
`83f83bb..v0.2.1`, which is what a patch release ought to look like and
is worth having checked. Compat rows earn `0.2.1`,
...
re-sync, since the queries are byte-identical at this bump, and the captures
...
`syntax/wolf.vim` already wrote `\x\{1,6}`, so the two editors disagreed,
and the amendment says vim was right.

Three pieces of prose were wrong rather than stale, and are fixed
instead of re-stamped:
...
- `wolf-mode.el`'s syntax table said "No character literal in wolf",
  ...
  unpainted literal is named as owed.
- `UPSTREAM.md`'s tree-sitter-wolf row described an empty seed repo and
  ...
  note. That file's own preamble is about this failure, so the
...
- `release-check` step 0 said "no tagged wolf-lang release exists",
  false since v0.1.0. Its blocker today is below.
...
2. The six captured editor smokes keep their recorded pins
   (`70bdd35`; `67c977f` for fackr) and refuse replay at `75fd2d0`, so
   release-check 3b is red on this until someone drives each
...
4. The fackr/facsimile patch series still have their re-verification
...
5. The astral-plane gap stays open, and it was measured here rather than
   repeated: a sweep of the whole 467-file corpus finds zero ZWJ and
...
6. The acquire step stays dark, for a NEW reason. Every earlier pin
   ...
   around. Only CI can make the three-OS server-lane claim (D35).
...
trunk this wave, after this pin. The editors do NOT
...
which is expected and not a regression. (`Point { x y z }` and
`(a b)`, the other laxities D67 names, already fail to parse in the
pinned grammar, so one spelling changes for the editors.) D67
...
its own last comment says it stays open. Flagged on the issue and not
...
paint it. Trunk CI was red since the le02 merge, which flipped the
...
2. The six captured editor smokes keep their recorded pins
   (`70bdd35`; `67c977f` for fackr) and refuse replay at `83f83bb`, so
   release-check 3b is red on this until someone drives each
   real editor again. Re-capturing them was not this sprint's act.
...
4. The fackr/facsimile patch series still have their
...
6. The acquire step stays dark until wolf-lang publishes an
   ...
   not at it), and only CI can make the three-OS server-lane claim
   (D35).
...
- ls01: `lspconf`, the conformance harness: recorded JSON-RPC
  ...
  `onetruth`, which checks LSP diagnostics == build diagnostics per
  sample (D34 made checkable; it found and closed DIV-LSP-001).
...
- ls06: the config tier: Helix and Emacs (eglot) exercised with
  ...
  documented with a NEVER stamp; `docs/MATRIX.md` with
