...
the same code that compiles (D34: one process, one truth). **This repo contains
no server code, and it never will.**

Not a fallback. Not a "temporary" diagnostic parser. Not a formatting shim. Not
a client-side approximation of a capability that has not shipped yet. A second
implementation of anything server-side here is the exact failure this track
exists to avoid: the moment the editor layer can answer a question itself, it
stops proving that the compiler can.

Concretely:

- **A capability an editor wants is a wolf-lang sprint, not a shim here.**
  Semantic tokens, inlay hints, rename, refactoring — all compiler-track
...
- Every behaviour worth having is worth a test; every bug fix arrives with the
  test that would have caught it. Tests land in the same commit as the code.
...
Snapshots hold the **normalized** view of a transcript, never the raw one — a
diff is then a behavior change rather than a different run.
...
sample**, never fork one into a local fixture, never "fix" one that looks
...
- **Line endings are protocol surface.** `.gitattributes` pins
  ...
  byte offsets; a CRLF checkout silently shifts every column in every
...
- **`Content-Length` counts bytes, not characters.** The single most common
  framing bug in hand-written clients, which is most of the clients this track
  integrates with.

## Toolchain

Pinned in `rust-toolchain.toml` (rustup/CI) and `rust-version` (everyone). Bump
deliberately, in a dedicated commit, CI-green on all three platforms.
...
`CLAUDE.md` are gitignored), as on the compiler and interpreter tracks.
...
Green, on a clean tree. `lspconf doctor` reporting `SERVER UNAVAILABLE` is the
expected state until wolf-lang ships `wolf lsp` (s52) — that is a skip, not a
failure, and `xtask ci` treats it as one.

## Where the plan lives

The sprint plan is the `lsp` track (`lsNN`) in the wolf metarepo. Sprint files
are the implementation contract; the decision log is the design authority.
