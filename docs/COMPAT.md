...
|---|---|---|---|---|---|---|---|
| [wolf-mode.el](../clients/emacs/README.md) | 0.0.1 | elisp file — clients/emacs/wolf-mode.el, also quoted verbatim in its README | 0.2.16 | 0.2.16 | `93a5fe5` | 2026-09-24 (kasumi (linux x86_64), against the ACQUIRED v0.2.16 x86_64-unknown-linux-gnu artifact (sha256 84e30c05…), not a local build — wave 46 puts every cargo invocation there. UNLIKE tl09, the PIN `version` string was read off THAT SAME linux artifact rather than the darwin one, because this lane could run it: `wolf --version` printed "wolf 0.2.16 (wolfgang, pin 93a5fe5)" directly. The darwin arm64 archive (sha256 b0431056…) was acquired and hashed member-by-member all the same, and both digests matched the release API's own before anything was unpacked) | none — the mode is a copy-pasteable snippet, and a snippet that phones a version check is a snippet nobody pastes |
| [helix config fragment](../clients/helix/README.md) | 0.0.1 | TOML fragment — clients/helix/languages.toml, copied into the user's config | 0.2.16 | 0.2.16 | `93a5fe5` | 2026-09-24 (kasumi (linux x86_64), against the ACQUIRED v0.2.16 x86_64-unknown-linux-gnu artifact (sha256 84e30c05…), not a local build — wave 46 puts every cargo invocation there. UNLIKE tl09, the PIN `version` string was read off THAT SAME linux artifact rather than the darwin one, because this lane could run it: `wolf --version` printed "wolf 0.2.16 (wolfgang, pin 93a5fe5)" directly. The darwin arm64 archive (sha256 b0431056…) was acquired and hashed member-by-member all the same, and both digests matched the release API's own before anything was unpacked) | none — a TOML fragment cannot run code; docs/COMPAT.md is the whole statement |
| [wolf.nvim](../clients/nvim/README.md) | 0.0.1 | plugin tree — clients/nvim/, published as the generated wolf.nvim mirror | 0.2.16 | 0.2.16 | `93a5fe5` | 2026-09-24 (kasumi (linux x86_64), against the ACQUIRED v0.2.16 x86_64-unknown-linux-gnu artifact (sha256 84e30c05…), not a local build — wave 46 puts every cargo invocation there. UNLIKE tl09, the PIN `version` string was read off THAT SAME linux artifact rather than the darwin one, because this lane could run it: `wolf --version` printed "wolf 0.2.16 (wolfgang, pin 93a5fe5)" directly. The darwin arm64 archive (sha256 b0431056…) was acquired and hashed member-by-member all the same, and both digests matched the release API's own before anything was unpacked) | :checkhealth wolf |
| [wolf (VS Code extension)](../clients/vscode/README.md) | 0.0.1 | vsix — clients/vscode/, packaged by @vscode/vsce | 0.2.16 | 0.2.16 | `93a5fe5` | 2026-09-24 (kasumi (linux x86_64), against the ACQUIRED v0.2.16 x86_64-unknown-linux-gnu artifact (sha256 84e30c05…), not a local build — wave 46 puts every cargo invocation there. UNLIKE tl09, the PIN `version` string was read off THAT SAME linux artifact rather than the darwin one, because this lane could run it: `wolf --version` printed "wolf 0.2.16 (wolfgang, pin 93a5fe5)" directly. The darwin arm64 archive (sha256 b0431056…) was acquired and hashed member-by-member all the same, and both digests matched the release API's own before anything was unpacked) | one notification at activation, plus `Wolf: Show Version` |
| [zed_wolf](../clients/zed/README.md) | 0.0.1 | wasm32-wasip2 component + manifest — clients/zed/ | 0.2.16 | 0.2.16 | `93a5fe5` | 2026-09-24 (kasumi (linux x86_64), against the ACQUIRED v0.2.16 x86_64-unknown-linux-gnu artifact (sha256 84e30c05…), not a local build — wave 46 puts every cargo invocation there. UNLIKE tl09, the PIN `version` string was read off THAT SAME linux artifact rather than the darwin one, because this lane could run it: `wolf --version` printed "wolf 0.2.16 (wolfgang, pin 93a5fe5)" directly. The darwin arm64 archive (sha256 b0431056…) was acquired and hashed member-by-member all the same, and both digests matched the release API's own before anything was unpacked) | none — the extension's wasm entry point is `language_server_command`, which Zed calls before any place a notification could be raised |

**`wolf-mode.el` caveat.** THE CAPTURED SMOKE IS STALE, AND IT IS NOW TWO PINS STALE. transcripts/emacs/smoke.jsonl was captured at 30731a6 (v0.2.14) and replay skips it at 0.2.16, so the part of this range a REAL emacs session earns is two versions behind the rest. The scripted suite — which covers every rung the smoke covers, through this client's own derived profile — did run at 93a5fe5. wolf-lsp#26.

**`helix config fragment` caveat.** THE CAPTURED SMOKE IS STALE, AND IT IS NOW TWO PINS STALE. transcripts/helix/smoke.jsonl was captured at 30731a6 (v0.2.14) and replay skips it at 0.2.16, so the part of this range a REAL helix session earns is two versions behind the rest. The scripted suite — which covers every rung the smoke covers, through this client's own derived profile — did run at 93a5fe5. wolf-lsp#26.

**`wolf.nvim` caveat.** THE CAPTURED SMOKE IS STALE, AND IT IS NOW TWO PINS STALE. transcripts/nvim/smoke.jsonl was captured at 30731a6 (v0.2.14) and replay skips it at 0.2.16, so the part of this range a REAL nvim session earns is two versions behind the rest. The scripted suite — which covers every rung the smoke covers, through this client's own derived profile — did run at 93a5fe5. wolf-lsp#26.

**`wolf (VS Code extension)` caveat.** THE CAPTURED SMOKE IS STALE, AND IT IS NOW TWO PINS STALE. transcripts/vscode/smoke.jsonl was captured at 30731a6 (v0.2.14) and replay skips it at 0.2.16, so the part of this range a REAL vscode session earns is two versions behind the rest. The scripted suite — which covers every rung the smoke covers, through this client's own derived profile — did run at 93a5fe5. wolf-lsp#26.

**`zed_wolf` caveat.** NO SESSION HAS EVER BEEN RECORDED. Zed has never been run against this extension by CI or by hand (docs/MATRIX.md). Its row inherits the range from the pin the other clients' scripted suites were verified at; a Zed session did not earn it — profiles/zed.json and transcripts/zed/smoke.jsonl are still owed. Read the row as 'the wasm builds and the config is statically consistent at this pin', which is the whole T2 claim. (tl11 briefly wrote the six-smokes caveat here by bulk edit and caught it: there is no transcripts/zed/smoke.jsonl to be stale, and a row claiming a capture that does not exist is worse than the gap it papers over.)
<!-- compat-table-end -->
