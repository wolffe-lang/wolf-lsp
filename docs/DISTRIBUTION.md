...
what has actually been proven about it, the command, and the human act that is
missing.
...
That is not a release channel, and the short retention is there so
...
collide with a `wolf-lang` release tag; a client version is the client's own
(ls07 non-target: no version lockstep).
...
versions have an odd minor and stable versions an even one: the marketplace
...
The same vsix goes to both registries in one job, so a version cannot ship to
one registry and be forgotten at the other. VSCodium, Cursor, Gitpod and
Eclipse Theia users get it at the same time.
...
Two properties are easy to lose in a later edit: the `if:` on the tag pattern,
and `environment:`. A GitHub environment confines the secrets to this one job
and can require a reviewer before it runs.
...
- [ ] **Replace the placeholder.** `clients/vscode/package.json` says
      `"publisher": "wolf-lang-unpublished"`, an invalid-looking value, so an
      accidental publish attempt fails on identity instead of succeeding under
      a name nobody chose. Change it in the same commit that first publishes,
      never earlier. `npm run test:manifest` reports it as owed until then.
...
- [ ] **Record custody as well as the token.** Who owns the account, where the
      ...
      by one credential on one laptop is lost with that laptop; two people must
...
- [ ] **Listing content**, which no program can produce:
      - a 128×128 PNG icon (`"icon": "images/icon.png"`), which is a design
...
(filetype detection, the LSP config, `:checkhealth wolf`, the help tags, and
both ls07 compatibility cases) passed from it.
...
no package registry, so "distribution" is a documentation problem.

Two consequences:

- The compatibility statement is not in the artifact. A TOML fragment cannot
  run code, so there is no runtime version check and there never will be.
  [`COMPAT.md`](COMPAT.md) is the whole statement.
- Upstreaming is possible and is not proposed. Helix ships language support
  ...
  built-in. That PR should not be opened before a public `wolf` exists, since
  an editor adding built-in support for a language nobody can install is asking
  for a revert. It is recorded as a future row, not an open one.
...
output lines and not on the exit code.
...
`profiles/zed.json`, stamp the matrix row, and *then* consider the registry.
...
its README so it can be pasted instead of installed. That is the shipping
channel today and it works with no infrastructure at all.

MELPA is a non-target at v1 (ls07). What it would require, so the decision can
...
fair trade for a real MELPA listing and a bad one for a hypothetical, so the
headers are absent.
...
Like Helix, this client has no runtime version check: a snippet a user pastes
into `init.el` that then compares versions against a declared range is a
snippet nobody pastes.
...
`clients/jetbrains/README.md` is a recipe for LSP4IJ and it has a
...
which is why neither has a `compat.json`; `cargo xtask compat-check` fails if
one appears.
...
- No `wolf` binary. No bundled server, no auto-download, no toolchain
  ...
  one publisher, and it is not this repository. Every client resolves a
  `wolf` the user already has, and says so when there is none.
