...
| [`wolffe-lang/tree-sitter-wolf`](https://github.com/wolffe-lang/tree-sitter-wolf) | nothing — the grammar lives in that repo's own trunk | `MERGED` | **Corrected at le04.** This row read "the repo is a seed commit (`b1b2c17`) with none" and called itself "the largest standing gap in the track" for three sprints after le02 wrote the grammar there; le03 and le04 have since extended it. Helix's `[[grammar]]` and Zed's `[grammars.wolf]` have been LIVE since le02 and are pinned here at `f704738`, so the sentence about commented-out blocks was false too. Nothing is owed upstream on this row: this repo consumes that grammar by pin, it does not submit to it |
